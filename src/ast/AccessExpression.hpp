...
public:
	//Constructors
	inline AccessExpression(UniquePointer<Expression>&& container, UniquePointer<Expression>&& accessor) : container(Move(container)), accessor(Move(accessor))
	{
	}
...
private:
	//Members
	UniquePointer<Expression> container;
	UniquePointer<Expression> accessor;
};