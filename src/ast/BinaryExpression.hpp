...
public:
	//Constructor
	inline BinaryExpression(Token op, Expression* left, Expression* right) : op(op), left(left), right(right)
	{
	}
...
private:
	//Members
	Token op;
	UniquePointer<Expression> left;
	UniquePointer<Expression> right;
};