let compute (_module: Semantic_ast.program_module) =
	...
	let add_ref name =
		if name = "self" then ()
		else Hashtbl.replace dependencies name ((Hashtbl.find dependencies name) +1)
	in
	
	let process_pattern pattern =
		match pattern with
		| Semantic_ast.Identifier id -> Hashtbl.add dependencies id 0
	...
	let rec process_expr expr =
		...
		| Semantic_ast.Function rules -> List.iter (process_rule) rules
