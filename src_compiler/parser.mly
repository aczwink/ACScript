...
%type <string> identifier_or_operator
...
expression:
	| pt=pattern SYMBOL_MAP expr=expression															{ Ast.Function( [(pt, expr)] ) }
	...
	| lhs=expression op=OPERATOR rhs=expression														{ Ast.Call( Ast.Select(lhs, op), Ast.Tuple(lhs::rhs::[]) ) }
...
function_rule:
	| pt=pattern SYMBOL_MAP expr=expression															{ (pt, expr) }
...
object_type_decl:
	| id=identifier_or_operator SYMBOL_COLON td=typedef SYMBOL_SEMICOLON { Ast.create_decl id td }
